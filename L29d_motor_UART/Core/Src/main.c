...
uint8_t rx_data;
uint16_t speed = 500; // 0–1000 (50% default)
...
{
  // Left motor stop/back, right forward (simple pivot)
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, GPIO_PIN_SET);
...
/* --- Apply PWM speed to both enables --- */
void motor_apply_speed(void)
{
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, speed); // PA8 EN1
  __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_2, speed); // PA7 EN2
}
...
int main(void)
{
  ...
  // Start UART interrupt reception (1 byte)
  HAL_UART_Receive_IT(&huart1, &rx_data, 1);
  /* USER CODE END 2 */
  /* USER CODE END 2 */
